...
score_sentiment <- function(docstr,posl,negl) {
    ...
    neg_words <- c(neg_words, paste(neg_words, "!", sep=""))
    ...
    # #### #
    # Exclamation Matching
    # #### #
    print("Starting Exclamation Matching :")
    excl_occurrences <- grep("!+", docvec, value=FALSE)

    # #### #
    # Sentiment Score Calculation
    # #### #
    sentiment_score <- avg_pos_score + avg_neg_score
    if (length(excl_occurrences) > 0 ) {
      sentiment_score <- sentiment_score*(1+length(excl_occurrences))
    }
