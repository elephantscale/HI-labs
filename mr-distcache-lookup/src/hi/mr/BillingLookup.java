...
public class BillingLookup extends Configured implements Tool
{
    public static void main(String[] args) throws Exception
    {
        int res = ToolRunner.run(new Configuration(), new BillingLookup(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception
    {
        ...
        job.setJarByClass(BillingLookup.class);
    ...
    public static class MyReducer extends Reducer<Text, IntWritable, Text, IntWritable>
    {
        ...
        protected void setup(Context context) throws IOException, InterruptedException
        {
            ...
            // TODO read in the file 'resource.properties'
            //this.resourceMappings.load(new FileReader("????"));
                ...
                InterruptedException
        {
            try
            {
                ...
                if (tokens.length  == 2)
                {
                    ...
                    String resourceName = "";
                    // TODO : lookup resourceID mapping, if none found then we default to 'Unknown'
                    //String resourceName = this.resourceMappings.getProperty(???, "Unknown");

                    int total = 0;
                    for (IntWritable cost : results)
                        total += cost.get();

                    // TODO : create a new Text key = customerID_resourceName
                    // write out the new key and total
                    //Text newKey = new Text (??? + "_" + ???)
